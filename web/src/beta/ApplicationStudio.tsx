...
const STEPS: Array<[Step, string]> = [["overview", "Job & location"], ["documents", "Documents"], ["questions", "Your answers"], ["review", "Final check"]];
...
function StudioSession({ session, job: summaryJob, onApplicationStatusChange }: Props) {
  ...
  const saveApplication = (status: BetaApplication["status"]) => act(async () => {
    if (status === "ready" && !canReady) throw new Error("Finish work rights, questions, and document review before marking ready.");
  ...
  const analyze = (kind: "rank" | "prepare") => act(async () => {
    ...
    if (kind === "rank") {
      ...
      await load(); setNotice("Role fit estimate saved. This is an AI estimate, not a hiring prediction or work-rights check.");
    } else {
  ...
  const recordStatus = serverReadiness?.application_id ? serverReadiness.application_status : application?.status ?? "Not created";
  return <section className="beta-application-studio workflow" aria-label="Application Studio">
    <header className="beta-application-studio__header">
      <p className="beta-eyebrow">Application Studio</p>
      <h2>{job.title}</h2>
      ...
      <p className="studio-status-line">Status: {recordStatus}. You always apply on the employer site yourself.</p>
      <details className="studio-about">
        <summary>More about this</summary>
        <p>Ready means you reviewed a current document packet against today's facts. Submitted means you told us you already applied outside this app — not verified by an employer. This app never submits for you.</p>
      </details>
    </header>
    ...
    {step === "overview" && <div className="workflow-stack">
      <form className="workflow-card" onSubmit={event => { event.preventDefault(); void act(async () => {
        ...
        invalidateReview(); setConfirmed(false); setEligibility(""); setReason(""); await load(); setNotice("Job description saved. Match score and work-rights answers for this role were cleared; older documents stay available to review.");
      }); }}>
        <h3>Job description</h3><p className="studio-lede">Paste the full posting. We do not open the link or check whether the role is still open.</p>
        ...
        <button className="beta-primary" disabled={busy || !dirty || !description.trim()}>Save job description</button>
      </form>
      <form className="workflow-card" onSubmit={event => { event.preventDefault(); void act(async () => {
        if (!eligibility || !confirmed || !reason.trim()) throw new Error("Choose an answer, add a short reason, and confirm.");
        ...
        invalidateReview(); await load(); setConfirmed(false); setNotice("Your answer was saved. It is not independently verified.");
      }); }}>
        <h3>Can you work in this location?</h3>
        <p className="studio-lede">Saved answer: {job.eligibility_review?.status ?? "Not recorded"}. {job.eligibility_review?.reason}</p>
        <p className="studio-helper">Answer from the posting and your work rights. We do not verify this for you.</p>
        <details className="studio-about">
          <summary>More about this</summary>
          <p>Neither AI nor a free-text answer can approve legal work rights. If you are unsure, choose “Not sure” and check with the employer or a qualified adviser.</p>
        </details>
        <label>Your answer<select required value={eligibility} disabled={busy || dirty} onChange={event => { setEligibility(event.target.value as typeof eligibility); setConfirmed(false); }}><option value="">Choose one</option><option value="eligible">Yes, I can work there</option><option value="ineligible">No, I cannot</option><option value="unknown">Not sure — I need to check</option></select></label>
        <label>Brief reason<textarea required maxLength={2000} value={reason} disabled={busy || dirty} onChange={event => { setReason(event.target.value); setConfirmed(false); }} placeholder="e.g. EU citizen, visa sponsorship needed, remote from UAE" /></label>
        <label className="workflow-check"><input type="checkbox" checked={confirmed} disabled={busy || dirty} onChange={event => setConfirmed(event.target.checked)} /><span>I confirm this answer for this role. It is not independently verified.</span></label>
        <button className="beta-primary" disabled={busy || dirty || !eligibility || !reason.trim() || !confirmed}>Save answer</button>
      </form>
      <div className="workflow-card"><h3>Role fit</h3><p className="studio-lede">{assessmentDisplay(job)}</p>{job.rationale ? <details className="studio-about"><summary>More about this</summary><p className="workflow-prose">{job.rationale}</p></details> : null}<button className="beta-primary" type="button" onClick={() => setStep("documents")}>Choose resume and continue</button></div>
    </div>}
    {step === "documents" && <div className="workflow-stack">
      <div className="workflow-card"><h3>Assess and prepare</h3><p className="studio-lede">Your resume, career facts, and saved job description set the context. Drafts only — not a hiring guarantee.</p>
        ...
        {!description.trim() && <p className="beta-notice">Add the full job description under Job &amp; location.</p>}{dirty && <p className="beta-notice">Save your edited job description first.</p>}
        ...
        {job.score != null && <><p>{assessmentDisplay(job)}</p>{job.rationale ? <details className="studio-about"><summary>More about this</summary><p className="workflow-prose">{job.rationale}</p></details> : null}</>}
      </div>
    ...
    {step === "questions" && <div className="workflow-stack"><h3>Facts needing your input</h3><p className="studio-lede">Answer only what you know. Saving does not rerun AI — return to Documents to assess or prepare again.</p>
      ...
      <button className="beta-secondary" type="button" onClick={() => setStep("documents")}>Back to Documents</button>
    </div>}
    {step === "review" && <div className="workflow-stack">
      <div className="workflow-card"><h3>Choose one document set</h3>
        <p className="studio-lede">Source: {selectedResume?.label ?? "Choose a source in Documents"}. Approach: {variant.replace(/_/g, " ")}. Resume and cover letter must be from the same generation and format.</p>
        ...
        <details className="studio-about">
          <summary>More about this</summary>
          <p>Each document set is tied to its generation, source resume, posting, and career facts. Changing those clears the review. Ready saves your review across devices — not a guarantee of accuracy, and not an employer submission.</p>
        </details>
      ...
      <div className="workflow-card"><h3>Check before you apply</h3><ul className="studio-checklist"><li>Can work in this location: {eligible ? "Yes (not independently verified)" : "Still needs your answer"}</li><li>Resume and cover letter for this set: {hasDocuments ? "Both saved" : "Not both available"}</li><li>Unanswered questions: {pending.length}</li></ul>
        ...
        <p className="studio-helper">Ready is your personal checklist — not a submission or approval.</p><div className="workflow-actions"><button className="beta-secondary" type="button" disabled={busy} onClick={() => void saveApplication("draft")}>Save draft</button><button className="beta-primary" type="button" disabled={busy || !canReady} onClick={() => void saveApplication("ready")}>Mark ready to apply</button></div>
      </div>
...
function QuestionForm({ question, disabled, onAnswer }: { question: BetaQuestion; disabled: boolean; onAnswer: (answer: string, remember: boolean) => Promise<void> }) {
  ...
  return <form className="workflow-card" aria-label={`Answer: ${question.prompt}`} onSubmit={event => { event.preventDefault(); if (answer.trim()) void onAnswer(answer.trim(), remember); }}>
    ...
    <button className="beta-primary" disabled={disabled || !answer.trim()}>Save answer</button>
  </form>;
}
