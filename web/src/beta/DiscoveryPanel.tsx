  ...
  return <section className="workflow discovery-panel" aria-labelledby="discovery-title">
    ...
    {result && <div className="workflow-stack discovery-results">
      ...
      {result.status === "unavailable" ? <p className="beta-error" role="alert">All configured discovery sources failed. This is not a successful search with no matches. Review source status below and search again later.</p>
        : <div className="workflow-card"><h2>{result.results.length ? "Roles to explore" : "No matching roles in the fetched sample"}</h2><p>{result.returned_count} of {result.matched_count} matching postings shown. {result.ranking === "profile_rules_v1" ? "Ordered by how well the posting overlaps your confirmed career facts — not an AI hiring score." : "Filtered by your saved preferences. Confirmed career evidence was not used for this ordering."} Checked {date(result.searched_at)}.</p><p>Results come only from the {result.sources.length} employer boards below, not the whole job market. Nothing is saved until you choose a role.</p><p className="discovery-provisional">{PROVISIONAL_NOTE}</p>{(result.partial || result.truncated || result.status === "partial") && <p className="beta-notice">Some boards failed, skipped records, or hit a limit. Treat this as a partial sample, not a complete market view.</p>}{!result.results.length && <><p>No suitable roles were found within these sources and your constraints. We have not changed your location or work preferences.</p><button className="beta-secondary" onClick={onEditPreferences}>Review search preferences</button></>}</div>}
      <details className="workflow-card discovery-sources" open={result.status !== "ok"}><summary>Board coverage ({result.sources.length})</summary><ul>{result.sources.map(source => {
        const skipped = source.dropped_count + source.unlisted_count + source.duplicate_count;
        const skipNote = skipped ? " · " + skipped + " skipped (incomplete, unlisted, or duplicate)" : "";
        const retryNote = source.retry_after ? " — wait at least " + source.retry_after + " seconds before trying it again" : "";
        return <li key={source.source}><h3>{source.source} <span className="discovery-chip">{source.status === "error" ? "Unavailable" : source.status === "partial" ? "Partial" : "Checked"}</span></h3><p>{source.cached ? "Used a recent cached check" : "Checked just now"}. Retrieved {date(source.fetched_at)}; checked {date(source.checked_at)}.</p><p>{source.returned_count} usable of {source.received_count} received{skipNote}.</p>{source.truncated && <p>Some posting text was truncated.</p>}{source.error_code && <p>This board had a problem{retryNote}.</p>}</li>;
      })}</ul></details>
      {result.results.map(job => {
        ...
        return <article className="workflow-card discovery-result" key={job.source_id} aria-label={`${job.title} at ${job.company_name}`}>
          ...
          {savedJob && <p>Saved in your private workspace. Opening Application Studio does not start AI or submit an application.</p>}
        </article>;
      })}
    </div>}
  </section>;
