...
class LaunchClosureContactMinimizationTests(OfflineCase):
    ...
    must keep the original values for authorized rendering. This is minimization
    of the model-processing path, not anonymization or end-to-end encryption.
    """
    ...
    def test_handoff_summary_email_and_phone_leave_the_model_payload(self):
        context = sample_context()
        context["profile"] = dict(
            context["profile"],
            display_name="Synthetic Tester",
            summary=("Contact Synthetic Tester at synthetic@example.invalid "
                     "or +971 50 123 4567"),
        )
        payload, facts = studio._context(context)
        sent = json.dumps(payload, ensure_ascii=False)
        self.assertNotIn("synthetic@example.invalid", sent)
        self.assertNotIn("971 50 123 4567", sent)
        summary = next(f["text"] for f in facts if f["id"] == "profile.summary")
        self.assertIn("[email removed]", summary)
        self.assertIn("[phone removed]", summary)
        self.assertNotIn("synthetic@example.invalid", summary)

    def test_context_filters_structured_fields_answers_chat_and_resume(self):
        context = sample_context()
        context["profile"] = dict(
            context["profile"],
        ...
        self.assertFalse(any(f["id"].startswith("profile.email") or f["id"].startswith("profile.phone")
                             for f in facts))
        provider = self.use_provider(chat_output())
        studio.answer_chat(context, "Email me at alex@example.org", "general",
                           [{"role": "user", "content": "My number is 971501234567"}])
        chat_sent = json.dumps(provider.calls[0]["payload"], ensure_ascii=False)
        for identifier in ("alex@example.org", "971501234567"):
            self.assertNotIn(identifier, chat_sent)
    ...
    def test_postal_addresses_remain_an_explicit_residual_risk(self):
        # Not claimed as anonymized. Street/postal forms are not stripped yet.
        text = studio._without_contact("Lives at 12 Baker Street, London NW1 6XE")
        self.assertIn("Baker Street", text)
        self.assertIn("NW1 6XE", text)
